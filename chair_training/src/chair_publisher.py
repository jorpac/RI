...
class ChairPub(object):
    def __init__(self):
        ...
        self.chairPub = rospy.Publisher("/robot2/cmd_vel", Twist, queue_size=10)


    def set_init_pose(self, init_pose):
        """
        Sets chair to initial position [0,0,0]
    ...
    def check_publishers_connection(self):
        ...
        while (self.chairPub.get_num_connections() == 0):
            rospy.logdebug("No susbribers to chairPub yet so we wait and try again")
            try:
                rate.sleep()
            except rospy.ROSInterruptException:
                # This is to avoid error when world is rested, time when backwards.
                pass
        rospy.logdebug("chairPub Publisher Connected")

        rospy.logdebug("All Joint Publishers READY")

    def move(self, msg):
        
        self.chairPub.publish(msg)
...
if __name__=="__main__":
    rospy.init_node('chair_publisher_node', log_level=rospy.WARN)
    joint_publisher = ChairPub()
    rate_value = 8.0
